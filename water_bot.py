...
async def status(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if r.status_code == 200:
        status1 = r.json()['switch1']
        status2 = r.json()['switch2']
...
async def turnon2(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await context.bot.send_message(chat_id=update.effective_chat.id, text=requests.get("http://localhost:8000/switch-on/2").json())
...
async def turnoff2(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await context.bot.send_message(chat_id=update.effective_chat.id, text=requests.get("http://localhost:8000/switch-off/2").json())
...
async def test1(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await context.bot.send_message(chat_id=update.effective_chat.id, text=requests.get("http://localhost:8000/switch-test/1").text.replace('"', '',2))
...
async def test2(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await context.bot.send_message(chat_id=update.effective_chat.id, text=requests.get("http://localhost:8000/switch-test/2").text.replace('"', '',2))
